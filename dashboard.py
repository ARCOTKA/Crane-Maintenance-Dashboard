...
if selected_tab == "📊 Detailed Analysis":
    ...
    if 'service_config' in st.session_state and not st.session_state.service_config.empty:
        tasks_df = st.session_state.service_config
        
            # Filter tasks dynamically based on the 'category' column
        if selected_entity.startswith('SP'):
            # For spreaders, only show tasks with the 'Spreader' category
            tasks_df = tasks_df[tasks_df['category'] == 'Spreader']
        else: # It's a crane
            # For cranes, show all tasks where the category is NOT 'Spreader'
            tasks_df = tasks_df[tasks_df['category'] != 'Spreader']

        # --- NEW 3-LEVEL DEPENDENT DROPDOWN LOGIC ---
        
        # 1. Get a sorted list of unique categories from the tasks_df
        categories = sorted(tasks_df['category'].unique())
        
        # LEVEL 1: Create the dropdown for selecting a Category
        ...
        # 2. If a category is selected, filter tasks for that category
        if selected_category:
            # Filter the dataframe to only include tasks for the chosen category
            category_tasks_df = tasks_df[tasks_df['category'] == selected_category]
            components = sorted(category_tasks_df['component'].unique())
            
            # LEVEL 2: Create the dependent dropdown for selecting a Component
            ...
            # 3. If a component is selected, filter tasks further
            if selected_component:
                # Filter again to only include tasks for the chosen component
                component_tasks_df = category_tasks_df[category_tasks_df['component'] == selected_component].sort_values('action_required')
                
                # LEVEL 3: Create the final dropdown for the service/inspection
