...
# MODIFIED: Now uses the centralized config.py for all directory and file paths.
# FIX: Corrected the path for TAG_IDS_FILE to use the provided 'TAG_SEARCH.txt'.
# FIX: Added robust value conversion to handle non-numeric data and prevent database errors.
...
# --- Use the updated config file and import the database module ---
try:
    # MODIFIED: Import Paths from the central config file for all paths.
    from config import Paths
    import database
except ImportError:
    # MODIFIED: Updated error message to reflect the single config.py dependency.
    print("ERROR: 'config.py' or 'database.py' not found. Please ensure they are in the same directory.")
    sys.exit(1)

# MODIFIED: Define path variables from the central Paths class.
LOG_DIRECTORY = Paths.LOG_DIRECTORY
DEBUG_LOG_FILE_PATH = Paths.DEBUG_LOG_FILE_PATH
TAG_IDS_FILE = Paths.TAG_IDS_FILE
TAG_CHANGE_FILE = Paths.TAG_CHANGE_FILE

# --- FIX: Override TAG_IDS_FILE to use the correct file name ---
# The config.py specifies 'tag_ids.txt', but the provided file is 'TAG_SEARCH.txt'.
# This line corrects the path to prevent a FileNotFoundError.
TAG_IDS_FILE = os.path.join(Paths.BASE_DIR, "TAG_SEARCH.txt")
...
logging.basicConfig(
    ...
    filename=DEBUG_LOG_FILE_PATH, # This now uses the path from config.py
            ...
            if match:
                ...
                if crane_match:
                    ...
                    if tag_detail_match:
                        ...
                        # --- FIX: Robust value conversion ---
                        # Attempt to convert value to float for DB insertion. Store NULL if it fails.
                        value_str = result_str.strip()
                        final_value = None # Default to None (for NULL in DB)
                        if value_str: # Only try to convert if the string is not empty
                            try:
                                final_value = float(value_str)
                            except ValueError:
                                logging.warning(f"  -> VALUE_CONVERSION_FAILED: Could not convert '{value_str}' to a number for tag '{human_readable_tag}'. Inserting NULL.")

                        try:
                            timestamp_obj = pd.to_datetime(timestamp_str, format='%Y-%m-%d_%H.%M.%S.%f', errors='coerce')
                            if pd.notna(timestamp_obj):
                                logging.debug(f"  -> SUCCESS: Parsed: Crane={crane_number}, Tag='{human_readable_tag}', Value='{final_value}'")
                                database.insert_stat(
                                    timestamp=timestamp_obj,
                                    crane_number=crane_number,
                                    tag_name=human_readable_tag,
                                    result=final_value # Use the converted float or None
                                )
                            else:
